...
if ( ! function_exists( 'scwd_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function scwd_entry_footer() {
...
function scwd_categorized_blog() {
	...
	if ( $all_the_cool_cats > 1 ) {
		...
		// This blog has only 1 category so scwd_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in scwd_categorized_blog.
 */
function scwd_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'scwd_categories' );
}
add_action( 'edit_category', 'scwd_category_transient_flusher' );
add_action( 'save_post',     'scwd_category_transient_flusher' );
